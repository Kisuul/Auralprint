...
This repo tracks **Auralprint** as a modular, offline-capable web app (“analyzer cosplaying as a visualizer”).
...
### Canonical artifacts (new repo)
This repo is modular, but keeps clear release mapping:

- `src/` — source modules (mutable implementation)
- `dist/` — static hosted build output
- `portable/` — file:// runnable bundle (no server required)
- `releases/Release_<N>/` — **frozen** package snapshot (what users download)
- Git tags: `v0.1.11`, `v0.1.13`, etc. mark canonical builds
...
- **Release 2** — **Build 111** (`v0.1.11`) — current canonical shipped baseline  
- **Release 3 (planned)** — **Build 113** (`v0.1.13`) — next shipped release
...
| 111 | v0.1.11 | R2 | ✅ Shipped (canonical) | Stability + stereo-orb baseline + band HUD |
| 112 | v0.1.12 | — | 🔧 In progress | Scrubber + Playlist/Queue |
| 113 | v0.1.13 | R3 | 🎯 Ship target | Recording / Capture export |
...
## Build 111 — v0.1.11 (Release 2, ✅ shipped canonical)
...
## Build 112 — v0.1.12 (🔧 milestone): “Scrubber + Queue”
...
- Auto-advance on track end (respect loop)
...
## Build 113 — v0.1.13 (Release 3 planned): Recording / Capture

**Goal:** export a session (visual output) to a video file while audio plays.

**Scope**
- Canvas capture via `MediaRecorder`
- **WebM first** (MP4 only if the browser supports it; otherwise WebM canonical)
- UI: Start/Stop recording + status timer + download link
- Capture settings exposed for future work (e.g., FPS, include-audio), via UX or documented code knobs

**DoD**
- Recording works while audio plays
- Output downloads cleanly
- No major FPS collapse at sane defaults
- Recording cleanup is correct (tracks stopped, URLs revoked, playback continues)
